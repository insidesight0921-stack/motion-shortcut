# 제스처 판정 규칙

> 모든 수치는 **튜닝 대상 초기값**이다. 실제 값은 `src/gesture/config.ts`가 기준이며, 개발 패널(화면 우하단 토글)에서 실시간으로 바꿀 수 있다.

## 기본 매핑 (Flickey 1차, 프로필의 슬라이드 모드)

제스처 5개(한 손) + 양손 X 는 고정이고, 어떤 명령에 연결할지는 프로필별·모드별로 "제스처" 화면에서 바꾼다(`localStorage` v3, D-017). 아래는 슬라이드 모드 기본값이다. 커서·레이저·자료 모드는 2·3차에서 채우며 지금은 빈 프리셋이다.

| 제스처 | 종류 | 기본 명령 | 위험도 |
|---|---|---|---|
| 오른쪽으로 스와이프 | 동적 | `slide.next` 다음 슬라이드 (→) | 낮음 |
| 왼쪽으로 스와이프 | 동적 | `slide.prev` 이전 슬라이드 (←) | 낮음 |
| 손바닥 펼치기 (0.5초 유지) | 정적 | `slide.blackout` 화면 가리기 (B) | 낮음 |
| 검지로 원 그리기 | 동적 | `slide.return` 발표 화면 복귀 (Esc) | 높음 |
| 주먹 쥐기 (1.2초 유지) | 정적 | `system.toggleEnabled` — 프로필 설정 `motionToggle === 'fist'` 일 때만 동작 | 고정, 편집 불가 |
| **양손 X (1초 유지)** | 양손 정적 | `MOTION OFF` 켜기/끄기 (= standby 토글) | 기본 켜기/끄기 경로 |

**MOTION OFF(= standby)**: 세션은 항상 여기서 시작한다(모션 기본 OFF, §19). 이 상태에서는 주먹을 포함한 모든 한 손 제스처가 무시되고, 켜는 경로는 양손 X·화면·키보드(`Ctrl+Shift+0`)뿐이다. 우선순위는 `MOTION OFF > 활성화 off(주먹 옵션일 때만) > 실행`.

주먹을 고정하는 이유(유지): 주먹은 한 손 잠금 해제 경로의 대안이다. 사용자가 다른 명령을 붙이거나 `none`으로 바꾸면 꺼 둔 활성화를 제스처로 다시 켤 방법이 없어진다. 저장소는 어떤 입력이 와도 주먹 항목을 고정값으로 덮어쓰고, 실제 토글은 인식 엔진이 수행한다. `fistHoldMs`는 1.2초 아래로 내리지 않는다(T-009).

### 양손 X 판정 (`src/gesture/twoHands.ts`, 추가 모듈)

- 손 두 개를 왼/오른으로 짝짓는다(`hand` 라벨, 없으면 손목 x). 라벨은 `vision/handedness.ts` 한 곳에서 MediaPipe 값을 뒤집어 만든다(실기 확인 항목 1).
- `x_cross`: 두 손의 손목(0)→중지 끝(12) 선분이 **교차**하고, 두 방향 벡터 사이 각도가 `crossAngleMinDeg`(40)~`crossAngleMaxDeg`(140), 손목 간 거리가 `crossMaxWristDistance`(2.5 손 크기) 이하. 손가락 모양은 묻지 않는다(주먹 X 도 X).
- `index_cross`(2차 모드 진입용, 판정만 준비): 두 손 모두 포인팅 + 검지 선분(5→8) 교차 + 같은 각도 조건. `x_cross`보다 먼저 검사한다.
- 점수 `0.5 + 0.5 × (각도가 범위 가운데에 가까움 0.6 + 교차점이 선분 가운데 0.4)`, `twoHandMinScore`(0.6) 이상이어야 후보.
- 토글은 `gesture/modeGesture.ts`의 `stepMotionToggle`: `xHoldMs`(1000) 유지 → 1회, 놓기 전 재토글 없음(release), `holdGraceMs` 유예, `cooldownMs` 쿨다운. 양손 포즈가 잡혀 있는 동안 한 손 실행은 "양손 제스처 진행 중"으로 무시된다.
- **실기 미확인**: 실제 X 자세에서 두 손이 서로 가리며 랜드마크가 흔들릴 때 점수가 유지되는지, 손목 간 거리 상한 2.5가 넉넉한지.

방향 기준은 사용자 시점이다(D-003). 거울 미리보기에서 보이는 방향과 같다.

## 좌표계

- HandLandmarker가 주는 21개 랜드마크의 x, y는 0~1 정규화 좌표(이미지 폭·높이 기준).
- `vision/mirror.ts`에서 `x → 1 - x`를 적용해 **사용자 시점 프레임**으로 바꾼다. 이후 모든 규칙에서 "오른쪽"은 사용자의 오른쪽이고, 화면(거울 미리보기)에서도 오른쪽이다.
- 랜드마크 인덱스: 0 손목, 1~4 엄지(4가 끝), 5~8 검지(5 MCP, 6 PIP, 7 DIP, 8 끝), 9~12 중지, 13~16 약지, 17~20 새끼.

## 정규화 (정적 포즈용)

1. 모든 점에서 손목(0)을 뺀다 → 손목이 원점.
2. 손목–중지 MCP(9) 거리를 1로 맞춘다 → 손 크기·카메라 거리 무관.

## 정적 포즈

코드: `src/gesture/static.ts`. 입력은 정규화 좌표.

### 손가락 펼침/접힘 (검지·중지·약지·새끼)

- 비율 `r = dist(끝, 손목) / dist(PIP, 손목)`
- `r ≥ fingerExtendRatio`(1.05) → 펼침. `r ≤ fingerFoldRatio`(0.95) → 접힘. 그 사이는 애매 구간(어느 쪽도 아님).
- 두 임계값을 다르게 둔 이유: 하나만 두면 경계에서 프레임마다 펼침/접힘이 깜빡인다(히스테리시스).

### 엄지 (D-004)

- 마진 `m = dist(엄지끝 4, 새끼MCP 17) − dist(엄지IP 3, 새끼MCP 17)`
- `m ≥ thumbExtendMargin`(0.1) → 펼침. 왼손/오른손, 거울 여부, 손 기울기에 무관하다.

### 손바닥 펼치기 `open_palm`

- 조건: 엄지 포함 5개 모두 펼침
- 점수: 손가락별 마진 `m = r − fingerExtendRatio`(엄지는 `thumbMargin − thumbExtendMargin`)를 `0.5 + 0.5·(m / poseSoftMargin)`으로 바꿔 0~1로 자르고 다섯 값을 **평균**한다(T-004). 모두 경계면 0.5, 모두 마진 ≥ `poseSoftMargin`(0.08)이면 1.0, 하나만 경계고 나머지가 확실하면 0.9.
- 상태 머신에서 `score ≥ minPoseScore`(0.6)이어야 후보로 인정.
- 개발 패널 "펼침 마진" 줄이 이 `m`의 원값이다. 어느 손가락이 0 근처인지 보고 튜닝한다.
- 유지 시간: `palmHoldMs` 500ms

### 주먹 `fist`

- 조건: 엄지 **제외** 4개 모두 접힘(`r ≤ fingerFoldRatio`). 엄지는 밖에 있든(따봉) 안에 있든 무관.
- 점수: 마진 `fingerFoldRatio − r`로 손바닥과 같은 식(평균). 개발 패널 "접힘 마진" 줄이 원값.
- 유지 시간: `fistHoldMs` 1200ms (2차 실기 후 2000에서 하향, T-009). 비활성 상태에서도 항상 인식된다(잠금 해제 경로)이므로 1200 아래로는 내리지 않는다.

### 검증된 것 / 안 된 것

- 합성 좌표 유닛 테스트로 확인: 위치·크기 불변, ±60° 기울기, 좌우 반전(왼손), 따봉=주먹, 엄지 접은 손≠손바닥, 검지만 편 손=없음.
- **실기 미확인**: 실제 카메라의 랜드마크 떨림, 손이 카메라를 비스듬히 볼 때(원근으로 손가락이 짧아 보임) 비율이 1 근처로 내려오는 문제. 이 경우 `fingerExtendRatio`를 낮추거나 `poseSoftMargin`을 넓히는 것이 첫 번째 튜닝 후보.

## 동적 제스처

코드: `src/gesture/dynamic.ts`. 입력은 `TrajectoryBuffer`의 프레임 열(사용자 시점, **정규화 전** 좌표). 거리는 창 안 평균 손 크기로 나눠 "손 크기 배수"로 비교한다.

공통 전제: 창 안에서 프레임 간격이 `trackingMaxGapMs`(150ms)를 넘거나, 마지막 프레임이 그만큼 오래됐으면(손 사라짐) 판정하지 않는다.

### 스와이프 `swipe_left` / `swipe_right`

- 관측점: 손바닥 중심(손목·네 MCP 평균). 손가락 모양과 무관하게 안정적.
- 창: 최근 `swipeWindowMs`(500ms), 최소 지속 `swipeMinDurationMs`(120ms), 최소 3프레임.
- 조건 (모두 만족):
  1. `|dx| ≥ swipeMinDistance`(2.0 손 크기). dx = 마지막 − 처음.
  2. 수직 편차: `max(|net dy|, 시작–끝 직선에서 벗어난 최대 수직 거리) / |dx| ≤ swipeMaxYRatio`(0.35)
  3. 직진성: `|dx| / Σ|Δx| ≥ swipeMinStraightness`(0.75). 왕복하면 낮아진다.
  4. 포인팅 손 비율 `≤ swipeMaxPointingFraction`(0.5). 검지만 편 손은 원을 그리는 손이므로 스와이프에서 제외.
- 방향: **dx > 0 → `swipe_right`(사용자 오른쪽)**. 좌표계가 이미 사용자 시점이라 부호 그대로다.
- 점수: `0.5 + 0.5 × (거리 여유 + 수평성 + 직진성)/3`. 조건을 통과하면 최소 0.5. 상태 머신은 `minDynamicScore`(0.5)와 비교.

### 원 `circle`

- 관측점: 검지 끝(8).
- 창: 최근 `circleWindowMs`(1500ms), 최소 8프레임.
- 조건:
  1. 궤적 중심(평균) 기준 **부호 있는** 누적 회전각 `|Σ Δθ| ≥ circleMinAngleDeg`(300°). Δθ는 (−180°, 180°]로 감아서 더한다. 좌우 흔들기는 +와 −가 상쇄되어 0 근처가 된다.
  2. 평균 반지름 `≥ circleMinRadius`(0.5 손 크기)
  3. 반지름 변동계수(표준편차/평균) `≤ circleMaxRadiusCv`(0.35)
  4. 궤적의 x 범위 / y 범위가 `circleMinAspect`(0.6)~`circleMaxAspect`(1.6) 사이 (T-008). 밀었다 되돌리는 납작한 고리를 거른다.
  5. 창 안 프레임의 `circleMinIndexExtendedFraction`(0.8) 이상에서 검지가 펼침 상태 (T-007). 0이면 끔. 주먹을 쥔 채 손목이 도는 것을 거른다.
- 방향은 명령에 쓰지 않지만 `detail.direction`에 남긴다(화면 좌표에서 양수 = 시계 방향).
- 점수: `0.5 + 0.5 × (회전각 여유 + 반지름 균일도)/2`.

### 원 vs 스와이프 충돌 처리

`detectDynamic` 순서: 완성된 원 → 원 진행 중이면(`|Σ Δθ| ≥ circleInProgressAngleDeg` 150°, 반지름 조건과 검지 게이트 통과. 가로/세로 비율은 반원이 본래 2:1이라 적용하지 않음) 스와이프 보류 → 스와이프.

반지름이 큰 원의 첫 90° 정도는 기하학적으로 "곡선 스와이프"와 같다(현 2.0 손 크기, 수직 편차 0.19). 회전각만으로는 못 막으므로 **손 모양**으로 구분한다: 원은 검지로 그리고, 스와이프는 펼친 손이나 자연스러운 손으로 한다. 펼친 손으로 큰 원을 그리면 첫 호가 스와이프로 잡힐 수 있다(알려진 한계, 테스트에 기록).

### 검증된 것 / 안 된 것

- 합성 궤적 유닛 테스트: 좌/우 스와이프 방향, 이동량 부족, 느린 이동, 대각선, 왕복, 추적 끊김, 손 크기 불변, 원 1바퀴(양방향), 반 바퀴, 좌우 흔들기 거부, 작은 원, 반지름 들쭉날쭉 거부, 검지 큰 원 중 스와이프 미발동, 포인팅 손 직선 이동은 스와이프 아님.
- **실기 미확인**: 사람 손의 실제 스와이프 속도(2 손 크기를 0.5초 안에 움직이는가), 원을 그릴 때 손목이 함께 도는지 아니면 손가락만 도는지(후자면 검지 끝 반지름이 작아 `circleMinRadius`를 내려야 할 수 있음), 원 그리는 동안 다른 손가락이 정말 접혀 있는지(아니면 `swipeMaxPointingFraction` 규칙이 안 먹음).

## 상태 머신

코드: `src/gesture/stateMachine.ts` (순수 함수 `step`), 합성은 `src/gesture/pipeline.ts`.

| 상태 | 의미 | 나가는 전이 |
|---|---|---|
| `idle` | 후보 없음 | 정적 포즈(점수 통과) → `holding` / 동적 제스처 → `armed` |
| `holding` | 정적 포즈 유지 중. HUD에 진행 링 | 유지 시간 충족 → `armed` / 포즈·손 소실이 `holdGraceMs` 초과 → `idle` + `ignored` / 동적 제스처 감지 → `cancelled` 후 `armed` |
| `armed` | "실행 예정" 표시 | `armDurationMs` 경과 → 실행(`executed` 이벤트) → `cooldown` |
| `cooldown` | `cooldownMs` 동안 모든 후보 무시 | 만료 → `idle`. 같은 제스처가 보여도 연장하지 않는다(T-003) |

기획서 7단계 대응: 1 후보 발견=`idle→holding/armed`, 2 유지·궤적=`holding`/`dynamic.ts`, 3 신뢰도=매 틱 `handScore ≥ minHandScore` 및 `poseScore ≥ minPoseScore`, 4 재실행 방지=쿨다운 연장 + release 규칙, 5 실행 예정=`armed`, 6 실행=`executed` 이벤트, 7=`cooldown`.

### 추가 규칙

- **release 규칙**: 정적 제스처(손바닥, 주먹)는 실행 후 그 포즈가 아닌 관측이 한 번은 있어야 다시 후보가 된다. 쿨다운을 0으로 내려도 손바닥을 계속 들고 있는 동안 재생/일시정지가 반복되지 않는다. 위반 시 `ignored:needs_release`. 재실행 방지는 이 규칙 하나가 맡는다(쿨다운 연장 규칙은 손을 편 채 스와이프할 때 쿨다운이 끝나지 않아 제거, T-003).
- **로그 스로틀**: 같은 (제스처, 사유)의 `ignored`는 에피소드당 1회에 더해 `ignoreLogThrottleMs`(500ms) 안에 다시 남기지 않는다(T-006).
- **속도 제한**: `holding` 중 손목 속도가 포즈별 한도를 넘으면 유지 시간이 쌓이지 않는다. 손바닥 `palmHoldMaxSpeed`(1.5 손 크기/초)는 손바닥을 편 채 스와이프할 때 재생 토글이 먼저 발동하는 것을 막기 위한 값이고, 주먹 `fistHoldMaxSpeed`(3.0)는 스와이프와 겹치지 않아 더 관대하다(T-009). 초과 시 `ignored:moving`.
- **동적 우선**: `holding` 중 동적 제스처가 감지되면 유지를 취소(`cancelled`)하고 동적 제스처를 실행한다.
- **fist 예외**: 활성화가 꺼져 있어도 `fist`는 항상 처리된다. 다른 4개는 `ignored:disabled`.
- **궤적 비우기**: 실행 직후 `TrajectoryBuffer.clear()`. 남은 궤적이 다음 틱에 같은 스와이프로 재검출되는 것을 막는다(D-008, 테스트로 고정).
- **로그 중복 방지**: 같은 후보가 계속 보이는 동안 같은 사유의 `ignored`는 한 번만 남는다. 후보가 사라졌다 다시 나타나면 다시 남는다.

### 무시 사유 목록

| 사유 | 뜻 | 튜닝 힌트 |
|---|---|---|
| `hold_too_short` | 유지 시간 전에 포즈가 바뀜 | 자주 나오면 `palmHoldMs`/`fistHoldMs` 감소 또는 `holdGraceMs` 증가 |
| `hand_lost` | 유지 중 손이 사라짐 | 조명·프레임 밖 문제. `minHandDetectionConfidence`(vision) 조정 |
| `moving` | 유지 중 손이 너무 빨리 움직임 | 정지한 손인데도 나오면 `palmHoldMaxSpeed`/`fistHoldMaxSpeed` 증가 |
| `cooldown` | 쿨다운 중 다른 후보 | 의도된 무시. 답답하면 `cooldownMs` 감소 |
| `disabled` | 비활성 상태에서 fist 외 제스처 | 의도된 무시 |
| `low_confidence` | 손·포즈·동적 점수 중 하나가 미달. 비고에 `hand 0.55 < 0.60`처럼 어떤 점수가 어떤 임계값에 걸렸는지 적힘(T-005) | `hand`면 `minHandScore`(handedness 확률이라 0.5가 사실상 통과), `pose`면 개발 패널의 손가락 마진을 보고 `poseSoftMargin`·`fingerExtendRatio`, `dynamic`이면 `minDynamicScore` |
| `needs_release` | 정적 제스처 실행 후 포즈를 풀지 않음 | 의도된 무시 |

### 임계값 (초기값)

| 키 | 값 | 설명 |
|---|---|---|
| `palmHoldMs` | 500 | 손바닥 유지 |
| `fistHoldMs` | 1200 | 주먹 유지. 잠금 해제 경로라 1200 아래로 내리지 않는다(T-009) |
| `holdGraceMs` | 400 | 유지 중 끊김 유예 (주먹은 랜드마크가 자주 튐, T-009) |
| `palmHoldMaxSpeed` | 1.5 | 손바닥 유지 중 최대 속도 (손 크기/초) |
| `fistHoldMaxSpeed` | 3.0 | 주먹 유지 중 최대 속도 (손 크기/초) |
| `motionWindowMs` | 150 | 속도 계산 창 |
| `armDurationMs` | 200 | 실행 예정 표시 |
| `cooldownMs` | 1500 | 쿨다운 |
| `ignoreLogThrottleMs` | 500 | 같은 (제스처, 사유) 무시 로그 최소 간격 |
| `minHandScore` | 0.5 | 손 신뢰도(handedness 확률) 하한. 구조상 ≥0.5라 0.5는 통과(T-005) |
| `minPoseScore` | 0.6 | 정적 포즈 점수 하한 |
| `minDynamicScore` | 0.5 | 동적 제스처 점수 하한 |
| `xHoldMs` | 1000 | 양손 X 유지 → MOTION OFF 토글 |
| `twoHandMinScore` | 0.6 | 양손 포즈 점수 하한 |
| `crossAngleMinDeg` / `crossAngleMaxDeg` | 40 / 140 | 교차 각도 범위 |
| `crossMaxWristDistance` | 2.5 | 교차 시 손목 최대 거리 (손 크기) |

## 알려진 오작동 사례

실기 테스트 전이라 **실제 사례는 아직 없다**. 아래는 설계 단계에서 예상한 것과 테스트로 드러난 것이다. 실기에서 로그를 복사해 이 절에 붙여 넣는다.

| 사례 | 상태 | 대응 |
|---|---|---|
| 펼친 손으로 큰 원(반지름 ≥ 1.2 손 크기)을 그리면 첫 호가 스와이프로 잡힘 | 테스트로 확인(D-010, T-002) | 원은 검지로 그린다. 검지 손이면 거부됨 |
| 손이 프레임 가장자리에서 들어올 때 스와이프처럼 보임 | 예상 | 추적 연속성(`trackingMaxGapMs`)과 최소 지속(`swipeMinDurationMs`)으로 1차 방어. 실기 확인 필요 |
| 카메라를 비스듬히 볼 때 손가락이 짧아 보여 손바닥이 `low_confidence` | 예상 | `fingerExtendRatio` 1.0으로 내리거나 `poseSoftMargin` 확대 |
| 손바닥을 든 채 살짝 흔들면 `moving`으로 유지가 끊김 | 예상 | `palmHoldMaxSpeed` 2.0~3.0으로 상향 |
| 주먹 유지가 랜드마크 튐·손 움직임으로 끊김 | 2차 실기 확인(0/3) | `fistHoldMs` 1200, `holdGraceMs` 400, `fistHoldMaxSpeed` 3.0으로 조정(T-009) |
| 주먹 시도 직후 원이 실행됨 | 2차 실기 확인 | 검지 펼침 게이트(T-007) |
| 스와이프 도중 납작한 고리 궤적이 원으로 잡힘 | 2차 실기 확인 | 가로/세로 비율 조건(T-008) |
| YouTube 플레이어 클릭 후 `Ctrl+Shift+M` 무반응 | 브라우저 제약 | 명령 실행 시 iframe blur. 버튼·주먹 사용 |

### 실기 로그 (여기에 붙여 넣기)

```
(비어 있음)
```

## 튜닝 방법

튜닝은 카메라 앞에서 사람이 한다. 도구는 세 가지다.

1. **개발 패널** (화면 우하단 "개발 패널" 버튼): 모든 임계값 슬라이더와 프레임당 판정 수치(손가락 비율, 엄지 마진, 포즈 점수, 속도, 동적 판정 세부값). 값은 즉시 반영되고 새로고침하면 기본값으로 돌아간다. 좋은 값을 찾으면 "변경값 복사"로 JSON을 받아 `src/gesture/config.ts`의 `DEFAULT_CONFIG`에 반영한다.
2. **실행 로그** (하단): 실행·무시가 사유와 함께 남는다. "복사"로 전체를 텍스트로 가져와 Claude Code에 붙여 넣으면 어떤 임계값을 만질지 같이 볼 수 있다. 같은 로그가 브라우저 콘솔에 `[motion]` 접두사로도 찍힌다.
3. **HUD**: 유지 진행 링과 현재 후보. "감지"는 되는데 "유지 중"으로 안 넘어가면 점수 문제(`low_confidence`)이고, 유지 중 링이 자꾸 리셋되면 유예/속도 문제다.

### 절차

1. 카메라 시작 → 개발 패널을 열고 손을 펴서 **손가락 비율**을 읽는다. 펼친 손이 1.2 안팎, 접은 손이 0.8 안팎이면 기본값이 맞다. 카메라를 비스듬히 볼 때 값이 1.0 근처로 붙으면 `fingerExtendRatio`를 1.0으로 내린다.
2. 손바닥을 들고 있어 본다. 링이 차오르는데 실행 직전에 `moving`으로 끊기면 `palmHoldMaxSpeed`를 올린다(주먹이면 `fistHoldMaxSpeed`). 손을 완전히 정지시키기 어려운 사람이 많다. 주먹은 `holdGraceMs`(400)로 랜드마크 튐을 흡수하며, `fistHoldMs`는 1200 아래로 내리지 않는다.
3. 오른쪽/왼쪽 스와이프를 10번씩 해 본다. 로그에 아무것도 안 남으면 이동량이 부족한 것(`swipeMinDistance` 내리기, 또는 더 크게). `low_confidence`가 남으면 점수 문제(`minDynamicScore` 내리기). 방향이 반대로 나오면 코드 문제다(D-003 위반) — 임계값으로 고치지 말 것.
4. 검지로 원을 그려 본다. 개발 패널 "동적" 줄에 `totalAngleDeg`가 얼마나 올라가는지 본다. 300에 못 미치면 창(`circleWindowMs`)을 늘리거나 각도를 낮춘다. `radiusCv`가 0.35를 넘으면 원이 찌그러진 것 — 상한을 올리거나 더 둥글게.
5. 원 그리다가 스와이프가 발동하면 다른 손가락이 펴져 있던 것이다. 개발 패널의 손가락 비율로 확인하고, 접기 어렵다면 `swipeMaxPointingFraction`을 올려 규칙을 완화하되 대신 원을 작게 그리게 한다(D-010).
6. 로그를 복사해 DECISIONS.md "시행착오"나 이 문서 "알려진 오작동 사례"에 붙여 넣는다.
