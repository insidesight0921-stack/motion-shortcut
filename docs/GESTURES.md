...
| 손바닥을 든 채 살짝 흔들면 `moving`으로 유지가 끊김 | 예상 | `holdMaxSpeed` 2.0~3.0으로 상향 |
| 주먹 2초가 너무 길어 중간에 랜드마크 튐으로 끊김 | 예상 | `holdGraceMs` 250으로 상향. 2초 자체는 기획 결정 |
...
2. 손바닥을 들고 있어 본다. 링이 차오르는데 실행 직전에 `moving`으로 끊기면 `holdMaxSpeed`를 올린다. 손을 완전히 정지시키기 어려운 사람이 많다.
