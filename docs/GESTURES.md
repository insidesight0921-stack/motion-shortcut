...
제스처 5개는 고정이고, 어떤 명령에 연결할지는 화면의 "제스처와 명령 연결" 편집기에서 사용자가 바꾼다(브라우저 `localStorage`에 저장, D-015). 아래는 기본값이다.

| 제스처 | 종류 | 기본 명령 | 파라미터 |
|---|---|---|---|
| 손바닥 펼치기 (0.5초 유지) | 정적 | `media.playPause` 재생/일시정지 | — |
| 오른쪽으로 스와이프 | 동적 | `media.seekForward` 앞으로 이동 | 10초 |
| 왼쪽으로 스와이프 | 동적 | `media.seekBackward` 뒤로 이동 | 10초 |
| 검지로 원 그리기 | 동적 | `timer.toggle` 타이머 시작/정지 | 3분 |
| 주먹 쥐기 (1.2초 유지) | 정적 | `system.toggleEnabled` 활성화 on/off | **고정, 편집 불가** |

주먹을 고정하는 이유: 주먹은 활성화가 꺼진 상태에서도 인식되는 **잠금 해제 경로**다. 사용자가 실수로 다른 명령을 붙이거나 `none`으로 바꾸면 꺼 둔 모션 단축키를 제스처로 다시 켤 방법이 없어진다. 편집기에서 잠금 표시되고, 저장소는 어떤 입력이 와도 주먹 항목을 고정값으로 덮어쓴다. 실제 토글은 명령이 아니라 인식 엔진이 수행하므로 매핑 계층이 깨져도 동작한다. 같은 이유로 `fistHoldMs`는 1.2초 아래로 내리지 않는다(T-009).
